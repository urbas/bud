...
namespace Bud.Dependencies {
  public static class DependenciesKeys {
    ...
    public static readonly TaskKey<ISet<Key>> ResolveInternalDependencies = new TaskKey<ISet<Key>>("resolveInternalDependencies").In(Dependencies);
    ...
    public static readonly TaskKey DependenciesClean = new TaskKey(BuildDirsKeys.Clean.Id, string.Format("Deletes the downloaded NuGet packages. You should invoke either '{0}' or '{1}' to download the packages again.", FetchDependencies, UpdateDependencies)).In(Dependencies);
  }
}