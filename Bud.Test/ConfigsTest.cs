...
namespace Bud {
  public class ConfigsTest {
    ...
    public void Const_defines_a_constant_valued_configuration() {
      Assert.AreEqual(42, Configs.Empty.Const(A, 42).Get(A));
    }

    [Test]
    public void Const_redefines_configurations() {
      Assert.AreEqual(1, Configs.Empty.Const(A, 42).Const(A, 1).Get(A));
    }

    [Test]
    public void InitConst_defines_a_constant_valued_configuration() {
      Assert.AreEqual(42, Configs.Empty.InitConst(A, 42).Get(A));
    }

    [Test]
    public void InitConst_does_not_redefine_configurations() {
      Assert.AreEqual(42, Configs.Empty.InitConst(A, 42).InitConst(A, 1).Get(A));
    }

    [Test]
    public void Redefined_configuration_is_never_invoked() {
      var intConfig = new Mock<Func<IConfigs, int>>();
      Assert.AreEqual(1, Configs.Empty.Init(A, intConfig.Object).Const(A, 1).Get(A));
      intConfig.Verify(self => self(It.IsAny<IConfigs>()), Times.Never());
    }

    [Test]
    public void Modified_configurations_modify_old_values() {
      var value = Configs.Empty.InitConst(A, 42).Modify(A, (configs, oldConfig) => oldConfig + 1).Get(A);
    ...
    public void Throw_when_requiring_the_wrong_value_type() {
      var exception = Assert.Throws<ConfigTypeException>(
        () => Configs.Empty.Const(AString, "foo").Get(A));
    ...
    public void Defining_and_then_invoking_two_configurations_must_return_both_of_their_values() {
      var configs = Configs.Empty.Const(A, 42).Const(B, 1337);
    ...
    public void Invoking_a_single_configuration_must_not_invoke_the_other() {
      var intConfig = new Mock<Func<IConfigs, int>>();
      var configs = Configs.Empty.Set(A, intConfig.Object).Const(B, 1337);
      configs.Get(B);
      intConfig.Verify(self => self(It.IsAny<IConfigs>()), Times.Never);
    }
    ...
    public void Extended_configs_must_contain_configurations_from_the_original_as_well_as_extending_configs() {
      var originalConfigs = Configs.Empty.Const(A, 42);
      var extendingConfigs = Configs.Empty.Const(B, 58);
    ...
    public void Invoking_an_undefined_config_must_throw_an_exception() {
      var actualException = Assert.Throws<ConfigUndefinedException>(() => Configs.Empty.Get(AString));
      Assert.That(actualException.Message, Contains.Substring(A));
    }

    [Test]
    public void A_config_can_invoke_another_config() {
      var configResult = Configs.Empty.Const(A, 42)
    ...
    public void Configurations_are_invoked_once_only_and_their_result_is_cached() {
      var intConfig = new Mock<Func<IConfigs, int>>();
      Configs.Empty.Set(A, intConfig.Object)
    ...
    public async void Multithreaded_access_to_configs_should_not_result_in_duplicate_invocations() {
      var intConfig = new Mock<Func<IConfigs, int>>();
      var configs = Configs.Empty.Set(A, intConfig.Object)
    ...
    public void Nesting_prefixes_config_names() {
      var nestedConfigs = "bar" / Configs.Empty.Const(A, 42);
      Assert.AreEqual(42, nestedConfigs.Get("bar" / A));
    }

    [Test]
    public void Nesting_allows_access_to_sibling_configs() {
      var nestedConfigs = "bar" / Configs.Empty.Set(B, configs => A[configs] + 1)
                                         .Const(A, 42);
    ...
    public void Nesting_with_multiple_modifications() {
      var nestedConfigs = "bar" / Configs.Empty
                                         .InitConst(C, 10)
                                         .Init(B, configs => 1)
    ...
    private static async Task<int> AddFooTwiceConcurrently(IConfigs tsks) {
