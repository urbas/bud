...
namespace Bud.IO {
  public class RecursiveDirFilesTest {
    ...
    public void List_files_in_the_folder() {
      ...
      Assert.That(new RecursiveDirFiles(noFileChanges, tempDir.Path, "*.txt"), Contains.Item(fileA));
    }

    [Test]
    public void Do_not_list_filtered_files() {
      tempDir.CreateEmptyFile("A", "A.txt");
      Assert.IsEmpty(new RecursiveDirFiles(noFileChanges, tempDir.Path, "*.cs"));
    }
    ...
    public void List_files_from_two_separate_subfolders() {
      ...
      Assert.That(new RecursiveDirFiles(noFileChanges, Path.Combine(tempDir.Path, "B"), "*.cs")
    ...
    public async void Observers_should_be_notified_directly_on_subscription() {
      var fileA = tempDir.CreateEmptyFile("A", "A.txt");
      var recursiveDirFiles = new RecursiveDirFiles(noFileChanges, tempDir.Path, "*.txt");
