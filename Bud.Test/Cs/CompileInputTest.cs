...
namespace Bud.Cs {
  public class CompileInputTest {
    ...
    public void FromInOut_throws_when_unknown_input_is_provided() {
      var exception = Assert.Throws<NotSupportedException>(() => ExtractInput(ImmutableList.Create(new TestInOut()), out sources, out assemblies, out dependencies));
    ...
    public void FromInOut_returns_empty_sources_when_given_empty_input() {
      ExtractInput(Empty<object>(), out sources, out assemblies, out dependencies);
    ...
    public void FromInOut_collects_sources() {
      using (var tempDir = new TemporaryDirectory()) {
        ...
        ExtractInput(new []{ fileA }, out sources, out assemblies, out dependencies);
    ...
    public void FromInOut_collects_assemblies() {
      using (var tempDir = new TemporaryDirectory()) {
        ...
        ExtractInput(new [] {ToAssembly(fileA)}, out sources, out assemblies, out dependencies);
        Assert.AreEqual(new[] {Files.ToTimestampedFile(fileA)},
                        assemblies);
      }
    }
    ...
    public void FromInOut_collects_compile_outputs_from_dependencies() {
      ...
      ExtractInput(new [] {dependency}, out sources, out assemblies, out dependencies);
